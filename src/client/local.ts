...
import type { RecognitionHooks, RecognitionPhase, SpeechRecognizer } from './webspeech.ts'
import { localModelSpec, langToWhisper, type LocalModelId } from './models.ts'
...
const pipelineCache = new Map<LocalModelId, AutomaticSpeechRecognitionPipeline>()
...
export async function downloadLocalModel(
  ...
  if (pipelineCache.has(id)) return
...
export function isLocalModelReady(id: LocalModelId): boolean {
  return pipelineCache.has(id)
}

/** Release a loaded model from memory (keeps the browser cache on disk). */
export function unloadLocalModel(id: LocalModelId): void {
